...
"""
Evaluate compression ratio of the peashooter tokenizer.
"""

import logging

from pealm import dataset, report, settings, tokenizer
...
def eval_peashooter_tokenizer(eval_settings: settings.EvalPeashooterTokenizer, report: report.DistReport) -> None:
    # the tokenizer was trained on data from earlier shards, so it has seen this data
    train_iter = dataset.ShardedParquet(
        eval_settings.input_dir,
        "train",
        shuffle=True,
        seed=eval_settings.seed,
    ).iter_texts_batch()
    val_iter = dataset.ShardedParquet(
        eval_settings.input_dir,
        "val",
    ).iter_texts_batch()
    train_docs: list[str] = next(train_iter)
    train_text: str = "\n".join(train_docs)
    val_docs: list[str] = next(val_iter)
    val_text: str = "\n".join(val_docs)
    ...
    tokenizer_results: dict[str, dict[str, dict[str, float]]] = {}
    vocab_sizes: dict[str, int] = {}

    for tokenizer_name in ["gpt2", "gpt4", "ours"]:
        if tokenizer_name == "gpt2":
            _tokenizer = tokenizer.PeashooterTokenizer.from_pretrained("gpt2")  # gpt-2 base model tokenizer
        elif tokenizer_name == "gpt4":
            _tokenizer = tokenizer.PeashooterTokenizer.from_pretrained("cl100k_base")  # gpt-4 base model tokenizer
        else:
            _tokenizer = tokenizer.PeashooterTokenizer.load(eval_settings.tokenizer_dir)

        vocab_sizes[tokenizer_name] = _tokenizer.vocab_size
        ...
        for name, text in all_text:
            ...
            if decoded != text:
                msg = f"tokenizer {tokenizer_name} failed to round-trip text type {name}"
    ...
    logger.info("vocab sizes: %s", vocab_sizes)
    ...
    # print comparisons
    print_comparison("GPT-2", tokenizer_results["gpt2"])
    print_comparison("GPT-4", tokenizer_results["gpt4"])
